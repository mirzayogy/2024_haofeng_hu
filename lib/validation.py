...
def getScore_flexible(img_or_path, filename = ""):
    """
    Hitung skor kualitas (UIQM, UCIQE) dari gambar.
    Input bisa berupa:
      - str (path file gambar)
      - PIL.Image.Image
      - numpy.ndarray (BGR atau RGB)

    Returns
    -------
    filename : str
        Nama file (jika input path) atau 'in-memory' (jika array/PIL).
    UIQM : float
    UCIQE : float
    """
    pil_img = None

    # Jika input path string
    if isinstance(img_or_path, str):
        pil_img = Image.open(img_or_path).convert("RGB")
        filename = os.path.basename(img_or_path)

    # Jika input sudah PIL
    elif isinstance(img_or_path, Image.Image):
        pil_img = img_or_path.convert("RGB")

    # Jika input numpy array
        ...
        raise TypeError("img_or_path harus path (str), PIL.Image.Image, atau numpy.ndarray")

    # Panggil fungsi skor
    UICM, UISM, UIConM, UIQM, UCIQE = getScore(pil_img)
    return filename, UIQM, UCIQE